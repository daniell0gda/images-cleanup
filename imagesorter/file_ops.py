...
def _collision_free_path(dest: Path) -> Path:
    ...
    Raises RuntimeError if no free candidate is found within _MAX_COLLISION_ATTEMPTS.
    """
    if not dest.exists():
        return dest
    stem = dest.stem
    ...
    for counter in range(1, _MAX_COLLISION_ATTEMPTS + 1):
        ...
        if not candidate.exists():
            ...
            return candidate
    raise RuntimeError(
        f"Could not find a free path for {dest} after {_MAX_COLLISION_ATTEMPTS} attempts"
    )
...
def transfer(src: Path, dest_dir: Path, copy: bool, on_collision: str = "rename") -> Path | None:
    ...
    """
    natural_path = dest_dir / src.name
    if len(str(natural_path.resolve())) > _MAX_PATH_LENGTH:
        raise OSError(
            f"Destination path exceeds {_MAX_PATH_LENGTH} characters: {natural_path}"
        )
