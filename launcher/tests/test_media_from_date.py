"""Tests for the GET /api/media ?from_date= seek parameter (Cluster 2).

Covers seeking pagination to the newest photo on or before a given ISO date,
unchanged behaviour when omitted, 422 on a malformed date, and an empty page
for a date older than every indexed photo.
"""
from __future__ import annotations

import os
from pathlib import Path

import yaml
from fastapi.testclient import TestClient
from PIL import Image


def _make_image(path: Path, size=(20, 20), color=(10, 120, 200)) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    Image.new("RGB", size, color).save(path)


def _build_app(tmp_path: Path, folders):
    configs = tmp_path / "configs"
    inbox = tmp_path / "inbox"
    configs.mkdir(exist_ok=True)
    inbox.mkdir(exist_ok=True)
    os.environ["CONFIGS_DIR"] = str(configs)
    os.environ["INBOX_BASE"] = str(inbox)
    os.environ["SYNC_DB"] = str(tmp_path / "sync.db")
    os.environ["MEDIA_DB"] = str(tmp_path / "media.db")
    os.environ["MEDIA_THUMBS_DIR"] = str(tmp_path / "thumbs")
    os.environ["MEDIA_PROXIES_DIR"] = str(tmp_path / "proxies")
    os.environ["MEDIA_PREVIEWS_DIR"] = str(tmp_path / "previews")
    (configs / "server.yaml").write_text(
        yaml.safe_dump({"media_library": {"enabled": True,
                                          "folders": [str(f) for f in folders],
                                          "schedule": "0 2 * * *"}}),
        encoding="utf-8",
    )
    import launcher.server as srv
    return srv.create_app(srv._JobState(), sync_detect_tags=lambda p: set())


def _trust(client: TestClient, device_id="dev-1") -> str:
    code = client.post(
        "/api/sync/devices", json={"device_id": device_id, "name": "Pixel"}
    ).json()["pairing_code"]
    client.post(f"/api/sync/devices/{device_id}/approve")
    return client.get(
        f"/api/sync/devices/{device_id}/status", headers={"X-Pairing-Code": code}
    ).json()["token"]


def _auth(token: str) -> dict:
    return {"Authorization": f"Bearer {token}"}


def _index(app) -> None:
    app.state.media_indexer._thumbnail = lambda row_id, path, kind: False
    app.state.media_indexer.build()


def _set_dates(app, dates_by_id: dict[int, str]) -> None:
    indexer = app.state.media_indexer
    ids = [r["id"] for r in indexer.list_all()]
    conn = indexer._conn()
    for idx, iso in dates_by_id.items():
        conn.execute("UPDATE media SET date_taken=? WHERE id=?", (iso, ids[idx]))
    conn.commit()


def test_from_date_starts_at_newest_on_or_before(tmp_path):
    root = tmp_path / "lib"
    for i in range(4):
        _make_image(root / f"a{i}.jpg")
    app = _build_app(tmp_path, [root])
    _index(app)
    _set_dates(app, {
        0: "2025-06-01T10:00:00+00:00",
        1: "2023-12-31T23:30:00+00:00",
        2: "2023-05-04T10:00:00+00:00",
        3: "2022-01-01T10:00:00+00:00",
    })
    client = TestClient(app)
    token = _trust(client)

    body = client.get("/api/media?from_date=2023-12-31", headers=_auth(token)).json()
    ids = [i["id"] for i in body["items"]]
    indexed = {r["id"]: r["date_taken"] for r in app.state.media_indexer.list_all()}
    # The 2025 photo is newer than the seek date and is excluded; the 2023-12-31
    # photo is the newest on or before the date and appears first, newest-first.
    dates = [indexed[i] for i in ids]
    assert dates == [
        "2023-12-31T23:30:00+00:00",
        "2023-05-04T10:00:00+00:00",
        "2022-01-01T10:00:00+00:00",
    ]


def test_from_date_omitted_unchanged(tmp_path):
    root = tmp_path / "lib"
    for i in range(3):
        _make_image(root / f"a{i}.jpg")
    app = _build_app(tmp_path, [root])
    _index(app)
    _set_dates(app, {
        0: "2025-06-01T10:00:00+00:00",
        1: "2023-12-31T23:30:00+00:00",
        2: "2022-01-01T10:00:00+00:00",
    })
    client = TestClient(app)
    token = _trust(client)

    without = client.get("/api/media", headers=_auth(token)).json()
    baseline = app.state.media_indexer.timeline(limit=100)
    assert [i["id"] for i in without["items"]] == [r["id"] for r in baseline]


def test_from_date_invalid_format_returns_422(tmp_path):
    root = tmp_path / "lib"
    _make_image(root / "a.jpg")
    app = _build_app(tmp_path, [root])
    _index(app)
    client = TestClient(app)
    token = _trust(client)

    assert client.get(
        "/api/media?from_date=not-a-date", headers=_auth(token)
    ).status_code == 422
    assert client.get(
        "/api/media?from_date=2024-13-40", headers=_auth(token)
    ).status_code == 422


def test_from_date_older_than_all_photos_empty(tmp_path):
    root = tmp_path / "lib"
    for i in range(2):
        _make_image(root / f"a{i}.jpg")
    app = _build_app(tmp_path, [root])
    _index(app)
    _set_dates(app, {
        0: "2025-06-01T10:00:00+00:00",
        1: "2024-01-01T10:00:00+00:00",
    })
    client = TestClient(app)
    token = _trust(client)

    body = client.get("/api/media?from_date=2020-01-01", headers=_auth(token)).json()
    assert body["items"] == []
    assert body["next_cursor"] is None
